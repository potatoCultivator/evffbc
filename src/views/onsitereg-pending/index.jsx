...
const PendingOnsite = () => {
    ...
    const [viewMode, setViewMode] = useState('church');
    ...
    useEffect(() => {
        ...
        const q = query(
            ...
            where('reg_type', '==', 'onsite')
        );

        const unsubscribe = onSnapshot(q, (snapshot) => {
            const groupedData = {};
            // Sort documents by date_added
            const sortedDocs = snapshot.docs.sort((a, b) => {
                const dateA = a.data().date_added?.toDate() || new Date(0);
                const dateB = b.data().date_added?.toDate() || new Date(0);
                return dateA - dateB;
            });
            ...
            sortedDocs.forEach((docSnapshot) => {
                ...
                if (viewMode === 'church') {
                    const churchKey = data.church || 'No Church';
                    if (!groupedData[churchKey]) groupedData[churchKey] = [];
                    groupedData[churchKey].push(data);
                } else {
                    // Each doc is its own "group"
    ...
    const ReferenceRow = ({ groupKey, registrants }) => {
        const isExpanded = expandedCode === groupKey;
        ...
        return (
            ...
            >
                <Card 
                    ...
                    onClick={() => setExpandedCode(isExpanded ? null : groupKey)}
                >
                    <Box sx={{ 
                        ...
                        <Grid container spacing={3}> {/* Increased spacing */}
                            <Grid item xs={12} md={6}>
                                <Box sx={{ mb: { xs: 2, sm: 0 } }}>
                                    ...
                                    >
                                        {viewMode === 'church'
                                            ? `Church: ${formatName(groupKey)}`
                                            : `Full Name: ${formatName(registrants[0]?.name || '')}`}
                                    </Typography>
                                    <Stack 
                        ...
                        <Grid item xs={12}>
                                ...
                                >
                                    ...
                                    <FormControl size="small" sx={{ minWidth: 140 }}>
                                        ...
                                        <Select
                                            ...
                                            label="Display By"
                                        >
                                            <MenuItem value="church">By Church</MenuItem>
                            ...
                            <Box sx={{ p: { xs: 2, sm: 3 } }}>
                                {Object.keys(filteredRegistrations).length === 0 ? (
                                    ...
                                    Object.entries(filteredRegistrations).map(([groupKey, registrants]) => (
                                        <ReferenceRow 
                                            key={groupKey} 
                                            groupKey={groupKey} 
