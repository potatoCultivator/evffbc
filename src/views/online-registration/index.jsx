...
import { uploadBatchConferees, registerOnline, getOnlineRegStatus } from '../Query';
...
const OnlineRegistration = () => {
    ...
    const REGISTRATION_FEE = 200; // Fee per person
    const [isRegistrationEnabled, setIsRegistrationEnabled] = useState(false);

    useEffect(() => {
        const unsubscribe = getOnlineRegStatus((status) => {
            setIsRegistrationEnabled(status);
        });
    ...
    const handleSubmitAll = async (e) => {
        ...
        try {
            ...
            if (!globalReferenceCode) {
                setReferenceError('GCash Reference Code is required');
                return;
            }
            ...
            const batchData = batch.map(person => ({
                ...
                referenceCode: globalReferenceCode.toLowerCase(),
                email: person.email.toLowerCase()
            }));

            console.log('Submitting batch:', batchData); // Debug log
            
            await uploadBatchConferees(batchData);
                ...
                <Card sx={{ 
                    ...
                    <CardContent sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
                        <form onSubmit={handleSubmitAll}>
                            {/* Add warning message before GCash reference input */}
                            <Box sx={{ mb: 2 }}>
                                <Typography 
                                    variant="subtitle1" 
                                    sx={{ 
                            ...
                            <Box sx={{ mb: 4 }}>
                                <TextField
                                    fullWidth
                                    label="GCash Reference Code for all registrants"
                                    value={globalReferenceCode}
                                    onChange={(e) => {
                                        setGlobalReferenceCode(e.target.value);
                                        setReferenceError('');
                                    }}
                                    error={Boolean(referenceError)}
                                    helperText={referenceError || "This reference code will be used for all registrants"}
